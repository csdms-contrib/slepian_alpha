function [I,dph,th,thR,th0,ph0]=sincos(th,m1,m2,dom,pars)
% [I,dph,th,nthR,th0,ph0]=sincos(th,m1,m2,dom,pars)
%
% Calculates the longitudinal integral 
% \int_{A}^{B}\sin(m1\phi)\cos(m2\phi)\.d\phi
% over a specified domain at some (Gauss-Legendre) integration point(s).
%
% INPUT:
%
% th         Colatitude(s) where you want it evaluated
...
for index=1:size(phint,2)/2
  ...
  if m1~=m2 & m1~=0 & m2~=0 & m1~=-m2
    I=I+1/2*(-cos((m1+m2)*B)*m1+...
	   cos((m1+m2)*B)*m2-...
	   cos((m1-m2)*B)*m1-...
	   cos((m1-m2)*B)*m2+...
	   cos((m1+m2)*A)*m1-...
  ...
  elseif m1~=m2 & m1==0 
    I=I+0;
  elseif m1~=m2 & m2==0 
    I=I-(cos(m1*B)-cos(m1*A))/m1;
  elseif (m1==m2 | (m1==-m2 & m1>0)) & m1~=0
    m=m1;
    I=I-1/2*(cos(m*B).^2-cos(m*A).^2)/m;
  elseif (m1==m2 | (m1==-m2 & m2>0)) & m1~=0
    m=m2;
    I=I+1/2*(cos(m*B).^2-cos(m*A).^2)/m;
  else
    I=I+0;
